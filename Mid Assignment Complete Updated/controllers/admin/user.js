...
router.get('/addMedicine', (req, res)=>{
    res.render('admin/user/addMedicine');
});
router.post('/addMedicine', [ 
    ...
    check('name', 'Medicine Name length should be 2 to 20 characters').matches(/^[A-Za-z\s]+$/)
                    .isLength({ min: 2}),

    check('comname', 'Company Name length should be 2 to 20 characters').matches(/^[A-Za-z\s]+$/)
                    .isLength({ min: 2}),

    check('genre', 'Genre length should be 2 to 20 characters').matches(/^[A-Za-z\s]+$/)
                    .isLength({ min: 2}),
   ...
   /* check('email', 'Email is not valid') 
    ...
    if (!errors.isEmpty()) {  
        ...
        res.render('admin/user/addMedicine', {
            ...
            userModel.validate2(user,(type) => {
                if ( type == req.body.name ) {
                    res.send("Name Already Taken")     
                }
                else {
                    console.log("Error");  
                    userModel.insert11(user, function(status){
